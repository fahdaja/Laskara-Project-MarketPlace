  ...
  <div class="min-h-screen bg-[#F9F7F7] flex p-6 gap-6 font-sans text-gray-800">
    ...
    >
      ...
      <div class="mt-8 flex flex-col gap-6 w-full items-center">
        ...
        >
          <img :src="user?.avatarUrl || `https://ui-avatars.com/api/?name=${user?.fullName || 'AV'}&background=1E3A8A&color=fff`" alt="User Profile" class="w-full h-full object-cover" v-if="sidebarCollapsed" />
          <img :src="user?.avatarUrl || `https://ui-avatars.com/api/?name=${user?.fullName || 'AV'}&background=1E3A8A&color=fff`" alt="User Profile" class="w-8 h-8 rounded-full object-cover" v-if="!sidebarCollapsed" />
          <span v-if="!sidebarCollapsed" class="font-semibold text-sm">{{ user?.fullName || 'Admin Validator' }}</span>
        </router-link>
